...
final class GitHubBuildManager: ObservableObject {
    ...
    func saveCredential() {
        do {
            ...
            guard !clean.isEmpty else {
                throw GitHubAPIError(message: "Enter a GitHub fine-grained token first.")
            }
    ...
    func build(project: StudioProject) async {
        ...
        do {
            status = "Uploading ReyForge project…"
            let encoder = JSONEncoder()
            encoder.outputFormatting = [.sortedKeys]
            encoder.dateEncodingStrategy = .iso8601
            let data = try encoder.encode(project)
            let upload = try await uploadProject(data, path: jobPath, jobID: jobID)
            uploadedSHA = upload.contentSHA
            ...
            let completed = try await wait(runID: run.id)
            guard completed.conclusion == "success" else {
                throw GitHubAPIError(
                    message: "GitHub build finished with: \(completed.conclusion ?? "unknown")"
                )
            }

            status = "Downloading generated IPA…"
            let artifact = try await findArtifact(runID: run.id, jobID: jobID)
            let (zipData, _) = try await request(
                path: "/repos/\(owner)/\(repo)/actions/artifacts/\(artifact.id)/zip"
            )

            status = "Extracting IPA…"
            ipaURL = try extractIPA(
                zipData: zipData,
                projectName: project.name,
                jobID: jobID
            )
            status = project.widget.enabled ? "App + Widget IPA ready" : "IPA ready"
        } catch {
            lastError = error.localizedDescription
            status = "Build failed"
        }
    ...
    private func request(
        ...
        guard let http = response as? HTTPURLResponse else {
            throw GitHubAPIError(message: "Invalid GitHub response.")
        }
        guard (200...299).contains(http.statusCode) else {
            let body = String(data: data, encoding: .utf8) ?? ""
            throw GitHubAPIError(
                message: "GitHub API \(http.statusCode): \(body.isEmpty ? "request failed" : body)"
            )
        }
        return (data, http)
    }
    ...
    private func uploadProject(
        ...
        guard let contentSHA = response.content?.sha,
            ...
            throw GitHubAPIError(message: "Project upload did not return commit metadata.")
        }
    ...
    private func findRun(commitSHA: String) async throws -> WorkflowRun {
        ...
        throw GitHubAPIError(message: "The ReyForge macOS build did not appear in GitHub Actions.")
    }

    private func wait(runID: Int64) async throws -> WorkflowRun {
        for _ in 0..<180 {
            let (data, _) = try await request(
    ...
    private func findArtifact(runID: Int64, jobID: String) async throws -> WorkflowArtifact {
        ...
        guard let artifact = envelope.artifacts.first(where: {
            $0.name == "ipa-\(jobID)" && !$0.expired
        }) else {
            throw GitHubAPIError(message: "The completed build did not expose its IPA artifact.")
        }
        return artifact
    }
    ...
    private func deleteProject(path: String, sha: String, jobID: String) async throws {
        let body: [String: Any] = [
            "message": "[skip ci] Clean ReyForge job \(jobID)",
            "sha": sha,
            "branch": jobsBranch
        ]
        _ = try await request(
    ...
    private func extractIPA(zipData: Data, projectName: String, jobID: String) throws -> URL {
        ...
        let zipURL = fileManager.temporaryDirectory.appendingPathComponent("reyforge-\(jobID).zip")
        ...
        let safeName = projectName
            ...
            .replacingOccurrences(of: ":", with: "-")
        let destination = buildsDirectory.appendingPathComponent(
            "\(safeName)-\(jobID.prefix(8)).ipa"
        )
