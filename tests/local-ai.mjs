...
try{
  ...
  await ai.getByRole('button',{name:'Load model · 469 MB',exact:true}).click();
  await page.waitForFunction(()=>ReyDev.localAI.state==='ready'||ReyDev.localAI.state==='error',null,{timeout:240000});
  assert.equal(await page.evaluate(()=>ReyDev.localAI.state),'ready',await ai.locator('[role=status]').innerText());
  checks.push('Default model downloads from its pinned URL and loads through the browser UI');
  await ai.getByRole('button',{name:'Unload',exact:true}).click();
  await page.waitForFunction(()=>ReyDev.localAI.state==='unloaded');
  await page.evaluate(()=>navigator.serviceWorker.ready);
  await page.waitForFunction(()=>!!navigator.serviceWorker.controller);
  await page.context().setOffline(true);
  await page.reload();await page.locator('#bootSkip').click();
  await page.evaluate(()=>openApp('ide'));await page.locator('.cm-content').waitFor();
  await ai.getByRole('button',{name:'Load model · 469 MB',exact:true}).click();
  await page.waitForFunction(()=>ReyDev.localAI.state==='ready'||ReyDev.localAI.state==='error',null,{timeout:180000});
  assert.equal(await page.evaluate(()=>ReyDev.localAI.state),'ready',await ai.locator('[role=status]').innerText());
  await ai.getByRole('textbox',{name:'Ask local AI'}).fill('What is 2 + 2? Reply with the number only.');
  await ai.getByRole('button',{name:'Send',exact:true}).click();
  await page.waitForFunction(()=>!ReyDev.localAI.busy&&!!document.querySelector('.dev-chat-answer pre'),null,{timeout:120000});
  const offlineResponse=await ai.locator('.dev-chat-answer pre').first().innerText();assert.match(offlineResponse,/\b4\b/);
  checks.push('Offline page restart reloads cached model weights and produces a real answer');
  await page.getByRole('button',{name:'Run program',exact:true}).click();
  await page.waitForFunction(()=>document.querySelector('.dev-output')?.textContent.includes('Exit code: 0'),null,{timeout:120000});
  assert.match(await page.locator('.dev-output').innerText(),/49/);
  checks.push('Saved AI-generated code executes with the network disabled after page restart');
  assert.deepEqual(errors,[]);
  await mkdir('.test-output',{recursive:true});
  await writeFile('.test-output/browser-ai-results.json',JSON.stringify({passed:checks,response,output,offlineResponse,errors},null,2));
